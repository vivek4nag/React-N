...
const AppLayout = () => {
  ...
  return (
    // the .provider used to make data or state available to all components in the component tree that consume the context. it wraps a portion of component tree(here the whole header to footer bcz userName is needed everywhere) and provides the context value to the components inside it. below we are using provider & passing a value which will overwrite the default value
